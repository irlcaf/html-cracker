...
generation_time_end = time.time() - generation_time_start
print("Average AnchorCAN generation time was: %f", generation_time_end/100)
#verifyAnchorCanData(current_anchor_random_number, can_id_key, can_id_initial_vector, ciphertext)
can_id_key = b'thisisjustakeeeythisisjustakeeey'
can_id_counter = get_random_bytes(1) #must be incremented
verify_time_start = time.time()
for i in range(0,100):
    verifyAnchorCanData(current_anchor_random_number, can_id_key, nonce, ciphertext)
verify_time_end = time.time() - verify_time_start
print("Average AnchorCAN verification time was: %f", verify_time_end/100)
# Test CAN library
#bustype = 'socketcan'
#channel = 'vcan0'
